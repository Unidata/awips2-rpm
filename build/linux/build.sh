...
# Sep 26, 2018  7488     dgilling    Add project gov.noaa.nws.ost.viz.stq.feature.
...
##################################
# Sync the AWIPS2 RadarServer Repo
##################################
if [ ! -z "$RADARSERVER_BRANCH" ]; then
   repo=$repo_dir/AWIPS2_RadarServer
   parts_to_sync=( 'common/*' 'edex/*' 'features/*' 'viz/*' 'rcm/*' 'rpms-RadarServer')
   $repo_dir/AWIPS2_build/build/common/sync_workspace.sh $repo $RADARSERVER_BRANCH $baseline ${parts_to_sync[*]}
   if [ $? -ne 0 ]; then
      exit 1
   fi

   ##################################
   # Create properties file for RCM
   ##################################
   echo "com.raytheon.uf.edex.rcm.feature" >> ${WORKSPACE}/baseline/build.edex/features.txt

   ##################################
   # Create properties file for RadarApps
   ##################################
   echo "com.raytheon.uf.viz.radarapps.feature" >> ${WORKSPACE}/baseline/build/features.txt
fi

##################################
# Sync the AWIPS2 nativelib Repo
##################################
if [ ! -z "$NATIVELIB_BRANCH" ]; then
   repo=$repo_dir/AWIPS2_nativelib
   parts_to_sync=( '.' )
   $repo_dir/AWIPS2_build/build/common/sync_workspace.sh $repo $NATIVELIB_BRANCH $baseline ${parts_to_sync[*]}
   if [ $? -ne 0 ]; then
      exit 1
   fi
fi
...
##################################
# Sync the rehost-irt Repo
##################################
if [ ! -z "$IRT_BRANCH" ]; then
   repo=$repo_dir/rehost-irt
   parts_to_sync=( '.' )
   $repo_dir/AWIPS2_build/build/common/sync_workspace.sh $repo $IRT_BRANCH $baseline ${parts_to_sync[*]}
   if [ $? -ne 0 ]; then
      exit 1
   fi
fi
...
if [ ! -z "$NWS_BRANCH" ]; then
   ...
   echo "gov.noaa.nws.ost.viz.stq.feature" >> ${WORKSPACE}/baseline/build/features.txt
   ...
   ##################################
   # Create properties file for GISDATASTORE
   ##################################
   echo "gov.noaa.nws.ocp.uf.viz.gisdatastore.feature" >> ${WORKSPACE}/baseline/build/features.txt

   ##################################
   # Create properties file for AWIPSREF
   ##################################
   echo "gov.noaa.nws.mdl.viz.awipsref.feature" >> ${WORKSPACE}/baseline/build/features.txt

   ##################################
   # Create properties file for DBGeo
   ##################################
   echo "gov.noaa.nws.obs.viz.geodata.feature" >> ${WORKSPACE}/baseline/build/features.txt
   echo "gov.noaa.nws.obs.edex.geodb.feature" >> ${WORKSPACE}/baseline/build.edex/features.txt

   ##################################
   # Create properties file for FIREWX
   ##################################
   echo "gov.noaa.nws.ocp.viz.firewx.feature" >> ${WORKSPACE}/baseline/build/features.txt

   ####################################
   # Create properties file for climate
   ####################################
   echo "gov.noaa.nws.ocp.viz.climate.feature" >> ${WORKSPACE}/baseline/build/features.txt
   ...
   echo "gov.noaa.nws.ocp.edex.psh.feature" >> ${WORKSPACE}/baseline/build.edex/features.txt

   ####################################
   # Create properties file for griddednucaps
   ####################################
...
if [ ! -z "$NCEP_BRANCH" ]; then
   ...
   parts_to_sync=( 'common/*' 'edex/*' 'features/*' 'viz/*' 'rpms-NCEP')
   $repo_dir/AWIPS2_build/build/common/sync_workspace.sh $repo $NCEP_BRANCH $baseline ${parts_to_sync[*]}
   if [ $? -ne 0 ]; then
      exit 1
   fi
   ##################################
   # Create properties file for NCEP
   ##################################
   echo "com.raytheon.uf.edex.nsbn.feature" >> ${WORKSPACE}/baseline/build.edex/features.txt
fi
...
if [ ! -z "$CIMSS_BRANCH" ]; then
   ...
   ##################################
   echo "edu.wisc.ssec.cimss.viz.probsevere.feature" >> ${WORKSPACE}/baseline/build/features.txt
   echo "edu.wisc.ssec.cimss.edex.probsevere.feature" >> ${WORKSPACE}/baseline/build.edex/features.txt
fi
...
if [ ! -z "$RPMS" ]; then
   ...
   if [ ! -z "$LIB_ENV_BUILD_PACKAGE" ]; then
      processBuildEnv ${WORKSPACE}/git/AWIPS2_build/build/linux/build.sh ${ORIGINAL_RPMS}
      if [ $? -ne 0 ]; then
         exit 1
      fi
   fi
