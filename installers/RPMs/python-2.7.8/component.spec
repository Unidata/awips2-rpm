...
%description
AWIPS II Python Distribution - Contains Python V2.7.8 plus modules
...
%build
PYTHON_TAR="Python-2.7.8.tgz"
PYTHON_PROJECT_SRC_DIR="%{_baseline_workspace}/foss/python-2.7.8/packaged/"
FOSS_PYTHON_DIR="%{_baseline_workspace}/foss/python-2.7.8/packaged/"
...
cd %{_python_build_loc}/Python-2.7.8
...
PYTHON_PROJECT_DIR="%{_baseline_workspace}/installers/RPMs/python-2.7.8"
...
pushd . > /dev/null
cd ${PYTHON_PROJECT_SRC_DIR}/%{_build_arch}
HDF5_TAR=`ls -1 ${HDF5184_PATTERN}`
popd > /dev/null

# Copy the hdf5 tar file to our build directory.
cp -v ${PYTHON_PROJECT_SRC_DIR}/%{_build_arch}/${HDF5_TAR} \
   %{_python_build_loc}
if [ $? -ne 0 ]; then
   exit 1
fi
...
FOSS_LAPACK_DIR="%{_baseline_workspace}/foss/lapack-3.4.2/packaged/"
...
cp -vP ${PYTHON_PROJECT_NATIVE_DIR}/%{_build_arch}/libjasper.so \
       ${PYTHON_PROJECT_NATIVE_DIR}/%{_build_arch}/libjasper.so.1 \
       ${PYTHON_PROJECT_NATIVE_DIR}/%{_build_arch}/libjasper.so.1.0.0 \
       %{_build_root}/awips2/python/lib
if [ $? -ne 0 ]; then
   exit 1
fi
...
if [ "%{_build_arch}" = "i386" ]; then
   /bin/tar -xvf ${PYTHON_PROJECT_SRC_DIR}/i386/awips2-python.tar \
