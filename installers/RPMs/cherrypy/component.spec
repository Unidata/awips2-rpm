...
Requires: awips2-python-cheroot
Requires: awips2-python-portend
Requires: awips2-python-more-itertools
Requires: awips2-python-zc.lockfile
Requires: awips2-python-contextlib2
...
%build
CHERRYPY_SRC_DIR="%{_baseline_workspace}/foss/cherrypy-%{version}/packaged"
cp -rv ${CHERRYPY_SRC_DIR}/CherryPy-%{version}.tar.gz %{_python_build_loc}
pushd . > /dev/null
cd %{_python_build_loc}
tar xf CherryPy-%{version}.tar.gz
cd CherryPy-%{version}
...
%install
pushd . > /dev/null
cd %{_python_build_loc}/CherryPy-%{version}
