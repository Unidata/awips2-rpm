...
/**
 * The Data Attribute Structure is a set of name-value pairs used to describe
 * the data in a particular dataset. The name-value pairs are called the
 * "attributes." The values may be of any of the DODS simple data types (DByte,
 * DInt32, DUInt32, DFloat64, DString and DURL), and may be scalar or vector.
 * (Note that all values are actually stored as string data.)
 * <p>
 ...
 * attributes. DODS uses this structure to provide information about variables
 * in a dataset.
 * <p>
 ...
 * example, the <code>lat</code> variable has an attribute <code>units</code> of
 ...
 * important to choose these names so a reader will know what they describe. In
 * the above example, the <code>GLOBAL</code> attribute provides information
 * about the entire dataset.
 * <p>
 * Data attribute information is an important part of the the data provided to a
 * DODS client by a server, and the DAS is how this data is packaged for sending
...
public class DAS implements Cloneable {
    ...
    /**
     * Returns a clone of this <code>DAS</code>. A deep copy is performed on all
     * <code>AttributeTable</code>s inside the <code>DAS</code>.
     * 
    ...
    public Object clone() {
        try {
            ...
            for (int i = 0; i < attr.size(); i++) {
                ...
                // clone element (don't clone key because it's a read-only
                // String)
     ...
     * <code>DAS</code>. Use the <code>getAttributeTable</code> method to get
     * the <code>AttributeTable</code> for a given name.
     * 
     ...
     * 
     * @param name
     *            the name of the <code>AttributeTable</code> to return.
     ...
     * 
     * @param name
     *            the name of the <code>AttributeTable</code> to add.
     * @param a
     *            the <code>AttributeTable</code> to add.
     * @see AttributeTable
     */
     ...
     * 
     * @param is
     *            the <code>InputStream</code> containing the <code>DAS</code>
     *            to parse.
     * @exception ParseException
     *                error in parser.
     * @exception DASException
     *                error in constructing <code>DAS</code>.
     * @exception dods.dap.parser.TokenMgrError
     *                error in token manager (unterminated quote).
