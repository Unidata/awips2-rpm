 ...
 * BaseTyeFactory <em>must</em> create instances of the SDtype classes, not the
 * Dtype classes. The is because we use the setRead method of the class
 * ServerMethods when creating constants (to ensure that the evaluator doesn't
 * try to read tem from the dataset!).
 * 
 * @author jhrg
 */
...
public class ExprParser implements ExprParserConstants {
    ...
    /**
     * Run the named projection function. Projection functions are run for their
     * side effect; the return value is discarded.
     * 
     * @param name
     *            The name of the projection function, look this up in the
     *            ServerDDS.
     * @param btv
     *            A vector of BaseType variables that are the arguments of the
     *            projection function.
     */
    ...
    /**
     * Remove double quotes from around a string. If there's not both start and
     * ending quotes, does nothing.
     * 
     * @param s
     *            The source string.
    ...
    /**
     * Given a stack of BaseType variables, mark these as part of the current
     * projection. This function assumes that if the TOS contains a Ctor type
     * variable, all of its members are to be projected. Also assume all
     * variables under the TOS are Ctor variables and only the ctor itself is to
     * be projected; the member within the Ctor that is part of the projection
     ...
     * 
     * @param ceEval
     *            A CEEvaluator instance where any select clauses will be dumped
     *            and from which the DDS, which supplies the evaluation
     *            environment, will be read.
     * @param factory
     *            A BaseTypeFactory used to generate instances of variables as
     *            needed (e.g., to hold values of constants).
     * @param clauseFactory
     *            A ClauseFactory used to generate instances of each type of
     *            clause.
     */
    final public void constraint_expression(CEEvaluator ceEval,
            BaseTypeFactory factory, ClauseFactory clauseFactory)
            throws ParseException, NoSuchVariableException,
            NoSuchFunctionException, InvalidOperatorException, DODSException,
    ...
    final public void selection() throws ParseException,
            NoSuchVariableException, NoSuchFunctionException,
    ...
    final public Clause bool_function() throws ParseException,
            NoSuchVariableException, NoSuchFunctionException,
            InvalidOperatorException, ParseException, DODSException {
    ...
    final public void projection() throws ParseException,
            NoSuchVariableException, NoSuchFunctionException,
            InvalidOperatorException, ParseException,
            InvalidParameterException, SBHException, WrongTypeException,
            DODSException {
        proj_clause();
        label_4: while (true) {
    ...
    final public void proj_clause() throws ParseException,
            NoSuchVariableException, NoSuchFunctionException,
            InvalidOperatorException, ParseException,
            InvalidParameterException, SBHException, WrongTypeException,
            DODSException {
    ...
    final public void proj_variable() throws ParseException,
            NoSuchVariableException, NoSuchFunctionException,
            InvalidOperatorException, ParseException,
            InvalidParameterException, SBHException, WrongTypeException,
            DODSException {
    ...
    final public Stack component(Stack components) throws ParseException,
    ...
    final public Vector arg_list() throws ParseException,
            NoSuchVariableException, NoSuchFunctionException,
            InvalidOperatorException, ParseException, DODSException {
    ...
    final public void array_index(int count, ServerArrayMethods bt)
            throws ParseException, ParseException, DODSException,
            InvalidParameterException, SBHException {
        Token t1, t2, t3;
        if (jj_2_4(5)) {
            ...
            try {
                bt.setProjection(count, Integer.parseInt(t1.image),
                        Integer.parseInt(t2.image), Integer.parseInt(t3.image));
            } catch (NumberFormatException e) {
                {
                    if (true)
                        throw new DODSException(DODSException.MALFORMED_EXPR,
                                "Could not parse one of " + t1.image + ", "
                                        + t2.image + ", " + t3.image
            ...
            case LBRACKET:
                ...
                try {
                    ...
                    {
                        if (true)
                            throw new DODSException(
                                    DODSException.MALFORMED_EXPR,
                                    "Could not parse " + t1.image
                                            + " as an integer: "
                                            + e.getMessage());
                    }
                }
    ...
    final public SubClause value() throws ParseException,
            NoSuchVariableException, NoSuchFunctionException,
        ...
        if (jj_2_6(2)) {
            ...
            try {
                {
                    if (true)
                        return clauseFactory.newDereferenceClause(((DURL) bt)
                                .getValue());
                }
            } catch (ClassCastException cce) {
                {
                    if (true)
                        throw new DODSException(
                                "Attempt to reference non-URL component "
                                        + bt.getName() + " as a URL.");
                }
            }
        } else if (jj_2_7(2)) {
    ...
    final public BaseType constant() throws ParseException, DODSException,
            ParseException {
        Token t;
        switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
        case INT:
            t = jj_consume_token(INT);
            // System.out.println("Setting constant value: "+t.image +
            // " (<INT>)");
    ...
    final private int[] jj_la1_0 = { 0x20000, 0x20000, 0x800000, 0x10000,
            0x13808000, 0x13908000, 0x13808000, 0xfe0, 0x10000, 0x800000,
            0x400000, 0x1000, 0x800000, 0x13808000, 0x10000, 0x1000, 0x400000,
            0x400000, 0x13800000, 0x400000, 0x13000000, };
    ...
    private void jj_add_error_token(int kind, int pos) {
        ...
        if (pos == jj_endpos + 1) {
            ...
            for (java.util.Enumeration enumi = jj_expentries.elements(); enumi
                    .hasMoreElements();) {
                int[] oldentry = (int[]) (enumi.nextElement());
