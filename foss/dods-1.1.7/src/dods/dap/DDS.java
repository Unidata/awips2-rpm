...
/**
 * The DODS Data Descriptor Object (DDS) is a data structure used by the DODS
 * software to describe datasets and subsets of those datasets. The DDS may be
 * thought of as the declarations for the data structures that will hold data
 * requested by some DODS client. Part of the job of a DODS server is to build a
 * suitable DDS for a specific dataset and to send it to the client. Depending
 * on the data access API in use, this may involve reading part of the dataset
 * and inferring the DDS. Other APIs may require the server simply to read some
 * ancillary data file with the DDS in it.
 * <p>
 * On the server side, in addition to the data declarations, the DDS holds the
 * clauses of any constraint expression that may have accompanied the data
 * request from the DODS client. The DDS object includes methods for modifying
 * the DDS according to the given constraint expression. It also has methods for
 * directly modifying a DDS, and for transmitting it from a server to a client.
 * <p>
 * For the client, the DDS object includes methods for reading the persistent
 * form of the object sent from a server. This includes parsing the ASCII
 * representation of the object and, possibly, reading data received from a
 * server into a data object.
 * <p>
 ...
 * structural information to the client. The same DDS can becomes a DataDDS when
 * data values are bound to the variables it defines.
 * <p>
 * For a complete description of the DDS layout and protocol, please refer to
 * <em>The DODS User Guide</em>.
 * <p>
 * The DDS has an ASCII representation, which is what is transmitted from a DODS
 * server to a client. Here is the DDS representation of an entire dataset
 * containing a time series of worldwide grids of sea surface temperatures:
 * 
 * <blockquote>
 * 
 * <pre>
 ...
 * </blockquote>
 * 
 * If the data request to this dataset includes a constraint expression, the
 * corresponding DDS might be different. For example, if the request was only
 * for northern hemisphere data at a specific time, the above DDS might be
 * modified to appear like this:
 * 
 * <blockquote>
 * 
 * <pre>
 ...
 * array. Note that the simple arrays (<em>lat</em>, <em>lon</em>, and
 * <em>time</em>) described in the dataset are also part of the <em>sst</em>
 * Grid object. They can be requested by themselves or as part of that larger
 * object.
 * <p>
 * See <em>The DODS User Guide</em>, or the documentation of the BaseType class
 * for descriptions of the DODS data types.
 * 
...
public class DDS implements Cloneable {
     ...
     * 
     * @param factory
     *            the server <code>BaseTypeFactory</code> object.
     */
    ...
    /**
     * Returns a clone of this <code>DDS</code>. A deep copy is performed on all
     * variables inside the <code>DDS</code>.
     * 
    ...
    /**
     * Get the Class factory. This is the machine that builds classes for the
    ...
    /**
     * Get the dataset's name. This is the name of the dataset itself, and is
     * not to be confused with the name of the file or disk on which it is
     * stored.
     * 
    ...
    /**
     * Set the dataset's name. This is the name of the dataset itself, and is
     * not to be confused with the name of the file or disk on which it is
     * stored.
     * 
     * @param name
    ...
    /**
     * Removes a variable from the <code>DDS</code>. Does nothing if the
     * variable can't be found. If there are multiple variables with the same
     * name, only the first will be removed. To detect this, call the
     * <code>checkSemantics</code> method to verify that each variable has a
     * unique name.
     * 
     * @param name
    ...
    /**
     * Is the variable <code>var</code> a vector of DConstructors? Return true
     * if it is, false otherwise. This mess will recurse into a DVector's
     * template BaseType (which is a BaseTypePrimivitiveVector) and look to see
     * if that is either a DConstructor or <em>contains</em> a DConstructor. So
     * the <code>List Strucutre { ... } g[10];</code> should be handled
     * correctly.
     * <p>
     * 
    ...
    private DConstructor isVectorOfDConstructor(BaseType var) {
        ...
        BaseTypePrimitiveVector btpv = (BaseTypePrimitiveVector) ((DVector) var)
                .getPrimitiveVector();
     ...
     * variable (or array/list of ctors) found on the top of the Stack
     * <code>compStack</code> (for component stack). When the named variable is
     * found, return the stack compStack modified so that it now contains each
     * ctor-type variable that on the path to the named variable. If the
     * variable is not found after exhausting all possibilities, throw
     * NoSuchVariable.
     * <p>
     * 
     * Note: This method takes the stack as a parameter so that it can be used
     * by a parser that is working through a list of identifiers that represents
     * the path to a variable <em>as well as</em> a shorthand notation for the
     * identifier that is the equivalent to the leaf node name alone. In the
     * form case the caller helps build the stack by repeatedly calling
     * <code>search</code>, in the latter case this method must build the stack
     * itself. This method is over kill for the first case.
     * 
     * @param name
     *            Search for the named variable.
     * @param compStack
     *            The component stack. This holds the BaseType variables that
     *            match the each component of a specific variable's name. This
     *            method starts its search using the element at the top of the
     *            stack and adds to the stack. The order of BaseType variables
     *            on the stack is the reverse of the tree-traverse order. That
     *            is, the top most element on the stack is the BaseType for the
     *            named variable, <em>under</em> that is the named variable's
     *            parent and so on.
     * @return A stack of BaseType variables which hold the path from the top of
     *         the DDS to the named variable.
     * @exception NoSuchvariableException.
     */
    ...
    private final class DDSSearch {
         ...
         * shallow search (see simpleSearch) and then look through all the ctor
         * variables. If there are no more ctors to check and the variable has
        ...
        boolean deepSearch(String name) throws NoSuchVariableException {
            ...
            BaseType start = components.empty() ? null : (BaseType) components
                    .peek();
     ...
     * 
     * @param is
     *            the InputStream containing the <code>DDS</code> to parse.
     * @exception ParseException
     *                thrown on a parser error.
     * @exception DDSException
     *                thrown on an error constructing the <code>DDS</code>.
    ...
    /**
     * Check the semantics of the <code>DDS</code>. If <code>all</code> is true,
     * check not only the semantics of the <code>DDS</code> itself, but also
     * recursively check all variables in the dataset.
     * 
     * @param all
     *            this flag indicates whether to check the semantics of the
     *            member variables, too.
     * @exception BadSemanticsException
     *                if semantics are bad
     */
    public void checkSemantics(boolean all) throws BadSemanticsException {
        if (name == null) {
            ...
            throw new BadSemanticsException(
                    "DDS.checkSemantics(): A dataset must have a name");
        }
     ...
     * 
     * @param os
     *            the <code>OutputStream</code> to use for output.
     ...
     * 
     * String s = ""; Enumeration e = getVariables();
     * 
     * //System.out.println("DDS.toASCII("+addName+")  getName(): "+getName());
     * while(e.hasMoreElements()){ BaseType bt = (BaseType)e.nextElement();
